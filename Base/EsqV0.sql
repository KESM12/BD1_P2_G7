...
CREATE UNIQUE INDEX HISTORIAL_AUDITORIAS__IDX ON HISTORIAL_AUDITORIAS
  (
    TRANSACCIONES_id_transaccion ASC
  )
  ;
ALTER TABLE HISTORIAL_AUDITORIAS ADD CONSTRAINT HISTORIAL_AUDITORIAS_PK PRIMARY KEY ( id_auditoria ) ;
...
ALTER TABLE TARJS_CREDS ADD CONSTRAINT TARJETAS_CREDITO_PK PRIMARY KEY ( id_tarjeta ) ;
...
ALTER TABLE TRANSACCIONES_INTERBANCARIAS ADD CONSTRAINT TRANS_INTERBANCARIAS_PK PRIMARY KEY ( id_interbancaria ) ;
...
ALTER TABLE HISTORIAL_AUDITORIAS ADD CONSTRAINT HIST_AUDITORIA_EMPLEADOS_FK FOREIGN KEY ( EMPLEADOS_id_empleado ) REFERENCES EMPLEADOS ( id_empleado ) ;

--  ERROR: FK name length exceeds maximum allowed length(30)
ALTER TABLE HISTORIAL_AUDITORIAS ADD CONSTRAINT HIST_AUDIT_TRANSACCIONES_FK FOREIGN KEY ( TRANSACCIONES_id_transaccion ) REFERENCES TRANSACCIONES ( id_transaccion ) ;

--  ERROR: FK name length exceeds maximum allowed length(30)
ALTER TABLE HISTORIAL_PRESTAMOS ADD CONSTRAINT HIST_PRESTAMOS_PRESTAMOS_FK FOREIGN KEY ( PRESTAMOS_id_prestamo ) REFERENCES PRESTAMOS ( id_prestamo ) ;
...
ALTER TABLE TARJS_CREDS ADD CONSTRAINT TARJS_CRED_CLIENTES_FK FOREIGN KEY ( CLIENTES_id_cliente ) REFERENCES CLIENTES ( id_cliente ) ;

ALTER TABLE TARJS_CREDS ADD CONSTRAINT TARJS_CRED_EST_TARJETAS_FK FOREIGN KEY ( EST_TARJETAS_id_estado ) REFERENCES EST_TARJETAS ( id_estado ) ;
...
--  ERROR: FK name length exceeds maximum allowed length(30)
ALTER TABLE TRANSACCIONES_INTERBANCARIAS ADD CONSTRAINT TRANS_INTERBAN_BOVEDAS_FK FOREIGN KEY ( BOVEDAS_id_boveda ) REFERENCES BOVEDAS ( id_boveda ) ;
